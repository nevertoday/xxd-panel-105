...
### 最も記憶に残すべき一場面だけを残し、削ること自体をデザインにする
...
上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 105 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## 向いている場面と解決できること

写真の本当の魅力は、写っている情報のすべてではなく、ある姿勢、関係、記憶に残したい一瞬に宿ることがあります。**Panel 105** は、そうした写真を静かで洗練された視覚的抜粋へ変えます。残す価値のあるものを見極め、写真を機械的になぞらず、紙片、色面、余白、印刷の肌理で再構成します。

### こんな場合に向いています

- **情報量の多い写真から、感情だけは失いたくないとき：** 一つの中心主体と、ごく少数の必要な補助要素を選びます。
- **呼吸できるポスターや表紙が必要なとき：** 一つの視覚的重心と大きな紙白で、主体、文字、沈黙のリズムをつくります。
- **滑らかなデジタル感より、実在する紙の触感がほしいとき：** 切り貼りの縁、粒子、刷り損じ、乾いた擦れ、インクのむら、わずかな版ずれを残します。
- **一つのスタイルを柔軟に納品したいとき：** 上下、左右、デザインのみ、複数比率、四端末壁紙、画像フォルダーの一括処理に対応します。

### 解決できる問題

- 雑然とした背景、重複する物、物語上の価値がない細部で画面を埋めません。
- 何を残し、何を削るかをすべて自分で決める必要がなく、モデルが美しさ、気品、識別性、構図価値から審美選択します。
- 色を均等に拾わず、最も記憶に残る関係を残して、柔らかく透明感のある調和した限定色へ浄化します。

### クイックスタート

> この画像を XXD Panel 105 で処理し、最適な構図とサイズを先に提案してください。
...
| 知りたいこと | このスタイルが与えるもの |
|---|---|
| **得られるもの** | 美しさと物語上の価値を持つ部分だけを残した、「記憶の抜粋」のような詩的紙本ポスター |
| **一目で分かる特徴** | 一つの視覚的重心、大きな紙白、切り紙の色面、不規則な紙端、粒子、わずかな版ずれ |
| **入力をどう尊重するか** | 主体の同一性、構造、姿勢、中心的な関係は識別可能なまま、情報密度と視覚組織を変えます |
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-105`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-105-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-105-prompt.zh-CN.md)
...
**特徴：** 知的な審美選択 · 詩的ミニマリズム · 紙本コラージュ · 一つの視覚的重心 · 大きな負の空間 · モノプリント／シルクスクリーン／Risograph 肌理 · 柔らかな限定色
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 105 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-105 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-105 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-105 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-105 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 105 を適用し、共通設定は一度だけ解決
/xxd-panel-105 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
XXD Panel の現行シリーズは 001〜112 まで更新され、各 Panel は独立した原文プロンプトと美的ロジックを保持しています。以下は本プロジェクト公開時点の履歴カタログで、001〜105 を連続して掲載し、現在のプロジェクトを太字で示します。
