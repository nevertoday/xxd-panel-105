...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 105 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。
...
```text
最も美しい一場面を見つける → 一つの中心主体を残す → 普通の情報と重複を削る → 紙片・色面・簡潔な幾何へ要約 → 切る・破る・重ねることで再構成 → 紙白・印刷粒子・小声の注記で「記憶の抜粋」を完成
```

## 完成作で識別しやすい特徴

- 一枚の写真を一つの視覚的重心に絞り、必要な補助要素だけを残して、それ以外を能動的な余白にします。
- 紙片、色面、簡潔な幾何へ要約しながら、主体、姿勢、中心的な関係を一目で識別できるように保ちます。
- mixed-media collage、cut-paper collage、monoprint、screen print、Risograph-inspired texture が紙本の触感をつくります。
- 元写真で最も美しく記憶に残る色関係から、主色、補助色、アクセント色、大きな紙白のリズムをつくります。
- 少量の文字は芸術出版物の小声の注釈のように働き、完全な場面再現、背景の充填、細部の堆積、カートゥーン化、商業イラスト感、テンプレート効果を避けます。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-105-prompt.en.md)
...
105 個の Panel は、それぞれ独立した原文プロンプトと美的ロジックを保持しています。表には 001 から 105 までを欠番なく掲載し、現在のプロジェクトを太字で示します。
...
| **兵士級** | `xxd-panel-NNN`（現在 001–105） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
