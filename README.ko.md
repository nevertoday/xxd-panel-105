...
# 🦁 XXD Panel 105｜시적 여백집

### 가장 기억할 가치가 있는 한 장면만 남기고, 덜어냄 자체를 디자인으로 만듭니다
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 105 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 105 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 105 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 105 Sample 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 105 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 105 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 105 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 105 additional top-bottom sample 4"></td>
  </tr>
</table>
...
위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 105가 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 어울리는 상황과 해결하는 문제

사진의 진짜 가치는 담긴 정보 전부가 아니라 하나의 자세, 관계, 혹은 기억에 남기고 싶은 순간에 있을 때가 있습니다. **Panel 105**는 그런 사진을 고요하고 세련된 시각적 발췌로 바꿉니다. 무엇을 남길 가치가 있는지 먼저 판단한 뒤 사진을 기계적으로 따라 그리지 않고 종잇조각, 색면, 여백, 인쇄 질감으로 재구성합니다.

### 이런 경우에 잘 맞습니다

- **정보가 많은 사진에서 감정만큼은 지키고 싶을 때:** 하나의 핵심 주체와 꼭 필요한 극소수의 보조 요소를 고릅니다.
- **숨 쉴 여백이 있는 포스터나 표지가 필요할 때:** 하나의 시각적 중심과 넓은 종이의 흰색으로 주체, 글자, 침묵에 리듬을 만듭니다.
- **매끈한 디지털 느낌보다 실제 종이 감촉을 원할 때:** 잘리고 찢긴 가장자리, 입자, 인쇄 누락, 마른 문지름, 고르지 않은 잉크, 미세한 색판 어긋남을 남깁니다.
- **하나의 스타일로 유연하게 납품해야 할 때:** 상하, 좌우, 디자인 전용, 여러 비율, 네 기기 배경화면, 이미지 폴더 일괄 처리를 지원합니다.

### 해결해 주는 것

- 어수선한 배경, 반복되는 사물, 서사 가치가 없는 세부로 화면을 가득 채우지 않습니다.
- 무엇을 남기고 덜어낼지 모두 직접 결정하지 않아도 됩니다. 모델이 아름다움, 분위기, 식별성, 구도 가치를 기준으로 미학적으로 선별합니다.
- 색을 고르게 표집하지 않고 가장 기억에 남는 관계만 남겨 부드럽고 맑으며 조화로운 제한색으로 정제합니다.

### 빠른 시작

> 이 이미지를 XXD Panel 105로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
| 궁금한 점 | 이 스타일이 제공하는 것 |
|---|---|
| **결과물** | 아름다움과 서사 가치가 있는 부분만 남긴 ‘기억의 발췌’ 같은 시적인 종이 포스터 |
| **대표 특징** | 하나의 시각적 중심, 넓은 종이의 흰색, 오려 붙인 색면, 불규칙한 종이 가장자리, 입자, 미세한 색판 어긋남 |
| **원본을 존중하는 방식** | 주체의 정체성, 구조, 자세, 핵심 관계는 식별 가능하게 유지하고 정보 밀도와 시각 조직을 바꿉니다 |
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-105`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-105-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-105-prompt.zh-CN.md)
...
**특징:** 지능적 미학 선별 · 시적 미니멀리즘 · 종이 콜라주 · 하나의 시각적 중심 · 넓은 네거티브 스페이스 · 모노프린트／실크스크린／Risograph 질감 · 부드러운 제한색


## 완성작에서 알아보기 쉬운 특징

- 한 장의 사진을 하나의 시각적 중심으로 좁히고 꼭 필요한 보조 요소만 남겨, 나머지는 능동적인 여백으로 만듭니다.
- 종잇조각, 색면, 간결한 기하로 요약하면서도 주체, 자세, 핵심 관계는 한눈에 알아볼 수 있게 유지합니다.
- mixed-media collage, cut-paper collage, monoprint, screen print, Risograph-inspired texture가 종이의 촉감을 만듭니다.
- 원본에서 가장 아름답고 기억에 남는 색 관계를 주조색, 보조색, 강조색, 넓은 종이의 흰색으로 구성합니다.
- 소량의 글자는 예술 출판물의 조용한 주석처럼 작동하며 장면 전체의 복제, 배경 채우기, 세부 누적, 카툰화, 상업 일러스트 느낌, 템플릿 효과를 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-105-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 105의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-105 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-105 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-105 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-105 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-105 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-105 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 105 적용, 공통 설정은 한 번만 확정
/xxd-panel-105 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
현재 XXD Panel 시리즈는 001–112까지 업데이트되었으며, 각 Panel은 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표는 이 프로젝트 공개 시점의 과거 목록으로 001–105을 연속해서 나열하며, 현재 프로젝트는 굵게 표시합니다.
