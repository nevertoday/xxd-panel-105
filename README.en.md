...
# 🦁 XXD Panel 105｜Poetic Space Anthology

### Keep the one moment worth remembering, and let subtraction become the design
...
The eight works above comprise four 16:9 landscape left–right samples and four 3:4 portrait top–bottom samples. Every work was generated independently by Panel 105 from its own original brief; no artwork from another numbered Panel is reused. The samples use intelligent English copy.

<!-- xxd-human-intro:start -->
## Best-fit situations and problems solved

Sometimes a photograph's real value is not everything it contains, but a gesture, a relationship, or one instant you want memory to keep. **Panel 105** turns that kind of source into a quiet, refined visual excerpt: it decides what deserves to remain, then rebuilds it through paper pieces, colour blocks, negative space, and print texture instead of mechanically tracing the photograph.

### Best for

- **A busy photograph whose feeling matters more than its detail:** select one core subject and only the few supporting elements that carry the memory.
- **A poster or cover that needs room to breathe:** use one visual centre and generous paper white to give subject, type, and silence a clear rhythm.
- **A tactile paper result rather than polished digital smoothness:** retain cut and torn edges, grain, misprints, dry rubbing, uneven ink, and slight registration shifts.
...
- Cluttered backgrounds, repeated objects, and narratively empty detail no longer fill the composition.
- You do not have to decide every keep-or-remove choice yourself; the model makes an aesthetic selection based on beauty, character, recognisability, and compositional value.
- Colour is not sampled evenly. The most memorable relationships are retained and purified into a soft, luminous, harmonious limited palette.

### Quick start

> Use XXD Panel 105 on this image and recommend the most suitable composition and size first.
...
**Signature:** intelligent aesthetic selection · poetic minimalism · paper collage · one visual centre · generous negative space · monoprint／screen print／Risograph texture · soft limited palette
...
|---|---|
| **What you get** | A poetic paper poster that reads like a “memory excerpt” and retains only what carries beauty and narrative value |
| **Recognisable signature** | One visual centre, generous paper white, cut-paper colour blocks, irregular edges, grain, and slight registration shifts |
| **How it respects the source** | Identity, structure, pose, and the core relationship remain recognisable; information density and visual organisation are what change |
...
```text
find the most beautiful moment → retain one core subject → remove ordinary and repeated information → reduce it to paper pieces, colour blocks, and simple geometry → rebuild through cutting, tearing, and layering → finish the “memory excerpt” with paper white, print grain, and quiet annotations
```

## Recognisable finished traits

- One photograph resolves into one visual centre with very few supporting elements; everything else becomes active blank space.
- Paper pieces, colour blocks, and simple geometry keep the subject, pose, and core relationship recognisable.
- mixed-media collage, cut-paper collage, monoprint, screen print, and Risograph-inspired texture create a tactile paper surface.
- Colour comes from the source's most beautiful and memorable relationships, structured as primary, supporting, accent, and generous paper white.
- Sparse words behave like quiet notes in an art publication; avoid full-scene reconstruction, filled backgrounds, detail accumulation, cartoonisation, commercial illustration, and template effects.

All aesthetic constraints live only in the [canonical prompt](references/original-prompt/zh-CN.md). The Skill and runtime adapters resolve delivery variables. [Skill workflow](SKILL.md) · [English runtime adapter](references/xxd-panel-105-prompt.en.md)
...
```text
/xxd-panel-105 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-105 photo.jpg --prefs last
/xxd-panel-105 photo.jpg --prefs edit
/xxd-panel-105 photo.jpg --prefs new
```
...
/xxd-panel-105 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB

# Left-right comparison, 3:2 landscape, no text
/xxd-panel-105 photo.jpg --mode left-right --size 3:2 --text none

# Design-only, 9:16 phone portrait, Japanese text
/xxd-panel-105 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# Linked wallpaper pack: establish one anchor, then recompose every device
/xxd-panel-105 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# Independent wallpapers with exact device resolutions
/xxd-panel-105 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# Follow the source image's aspect ratio
/xxd-panel-105 photo.jpg --mode design-only --size source --text none

# Create square, portrait, and landscape compositions
/xxd-panel-105 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# Combine a custom ratio with exact pixel targets
/xxd-panel-105 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# Let the original prompt generate wording; constrain only the locale
/xxd-panel-105 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-105 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-105 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 105 to a whole directory; shared settings resolve once
/xxd-panel-105 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB

# Repeated parameters accumulate; place the task under a chosen root
/xxd-panel-105 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
```bash
git clone https://github.com/nevertoday/xxd-panel-105.git
mkdir -p ~/.codex/skills
ln -s "$(pwd)/xxd-panel-105" ~/.codex/skills/xxd-panel-105
```

You can also install it directly with `npx skills`:

```bash
npx skills add https://github.com/nevertoday/xxd-panel-105 --skill xxd-panel-105
```
...
Claude Code users may link the same folder under `~/.claude/skills/xxd-panel-105`. Restart the agent session after installation.
...
Full specifications: [Skill workflow](SKILL.md) · [source archive](references/original-prompt/zh-CN.md) · [English runtime adapter](references/xxd-panel-105-prompt.en.md) · [Chinese runtime adapter](references/xxd-panel-105-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## Complete XXD Panel catalog

The current XXD Panel series runs from 001 through 112, and every Panel retains its own independent original prompt and aesthetic logic. The table below is the historical catalog as of this project’s release, listing 001 through 105; the current project is bold.
