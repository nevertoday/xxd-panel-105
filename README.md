...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 105 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。

<!-- xxd-human-intro:start -->
## 适用场景与解决的问题

有些照片真正动人的不是全部信息，而是一种姿态、一段关系，或某个只想留在记忆里的瞬间。**Panel 105** 适合把这样的照片做成安静而高级的视觉摘录：它先判断什么最值得留下，再用纸片、色块、留白和印刷肌理重构，不会把原图机械描摹成插画。

### 适合这些情况

- **照片信息很多，但你只想保住情绪：** 从复杂场景中选出一个核心主体与极少量必要陪衬。
- **想做有呼吸感的海报或封面：** 用单一视觉重心和大面积纸白，让主体、文字与留白形成节奏。
- **喜欢真实纸本而不是光滑数字感：** 保留剪贴边缘、颗粒、漏印、干擦、墨色不均和轻微套色偏差。
- **需要同一风格灵活交付：** 支持上下对照、左右对照、纯设计、多比例、四端壁纸和图片目录批量处理。

### 它替你解决什么

- 不再把杂乱背景、重复物件和无叙事价值的细节全部塞进画面。
- 不用自己决定哪些元素该留、该删；模型按照片的美感与识别度完成审美筛选。
- 配色不会平均取样，而是保留最有记忆点的颜色关系，再净化成柔和、通透、协调的有限色盘。

### 快速开始

> 用 XXD Panel 105 帮我处理这张图，先推荐最合适的构图和尺寸。
...
**关键词：** 智能审美筛选 · 诗意极简 · 纸本拼贴 · 单一视觉重心 · 大面积负空间 · 单版画／丝网／Risograph 肌理 · 柔和有限色盘

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 105 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一张像“记忆摘录”的诗意纸本海报，只留下最有美感和叙事价值的部分 |
| **一眼可辨的特点** | 单一视觉重心、大面积纸白、剪贴色块、不规则纸边、颗粒与轻微套色偏差 |
| **它如何尊重你的输入** | 主体身份、结构、姿态与叙事关系仍可辨认；改变的是信息密度与视觉组织 |
...
```text
识别最美的一幕 → 留下一个核心主体 → 删除普通与重复信息 → 概括成纸片、色块和简洁几何 → 用剪切、撕贴、叠压重组 → 以纸白、印刷颗粒和轻声注记完成“记忆摘录”
```

## 成品中最容易识别的风格特点

- 一张照片只保留一个视觉重心，必要陪衬极少，其余主动化为空白。
- 主体被概括成纸片、色块和简洁几何，但身份、姿态与核心关系仍能一眼辨认。
- mixed-media collage、cut-paper collage、monoprint、screen print 与 Risograph-inspired texture 共同形成纸本触感。
- 色彩从原图最美、最有记忆点的关系中提炼，以主色、辅助色、点睛色和大面积纸白建立节奏。
- 少量文字像艺术出版物里的轻声注释；拒绝完整场景复刻、背景填满、细节堆积、卡通化、商业插画感和模板化效果。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-105-prompt.en.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 105 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-105 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-105 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-105 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 105；公共设置只解析一次
/xxd-panel-105 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-105`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-105-prompt.en.md) · [中文运行适配器](references/xxd-panel-105-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全系列项目

105 个 Panel 各自保留独立的原始提示词与审美逻辑。下面连续列出 001–105 的全部项目地址和核心风格特点；当前项目以粗体标出。
...
| **士兵级** | `xxd-panel-NNN`（当前 001–105） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
...
<div align="center"><strong>先保留身份，再把它变成一眼记住的角色。</strong></div>
